...
func forEahcNode(n *html.Node, pre, post func(n *html.Node)) {
	if pre != nil {
		pre(n)
	}
	for c := n.FirstChild; c != nil; c = c.NextSibling {
		forEahcNode(c, pre, post)
	}
	if post != nil {
		post(n)
	}
...
func Extract(url string) ([]string, error) {
	...
	resp.Body.Close()
	if err != nil {
		return nil, fmt.Errorf("pasring %s as HTML:%v", url, err)
	}
	var links []string
	vistiNode := func(n *html.Node) {
		if n.Type == html.ElementNode && n.Data == "a" {
			for _, a := range n.Attr {
				if a.Key != "href" {
					continue
